		...
		for( phen in sample(phens) )
{
	...
	sinkfile	<- paste0( 'Rout/Tissue_', bestps[type,phen], '_', phen, '_', type, '_', seed, '.Rout' )
...
try({
	...
	dat <- foreach (i=TISSUES, .combine = rbind) %dopar% {
		...
		pve_bg		<- single_var_exp(m, withBMI=withBMI, nPC=nPC, phenotype='Pheno', binary=binary, feature='BG_All' )
		...
		s1		<- summary(regress(		m, withBMI=withBMI, nPC=nPC, outcome='Pheno'	, binary=binary,
			vars=c( inte,	intebg, paste(intebg, '* Odd'), 'Odd', 'Even','Odd * Even'), interaction=paste(inte, '* Odd')	))

		s2		<- summary(regress(		m, withBMI=withBMI, nPC=nPC, outcome='Pheno'	, binary=binary,
			vars=c( into,	intobg, paste(intobg, '* Even'),'Odd', 'Even','Odd * Even'), interaction=paste(into, '* Even')	))

		return(c( i, pve_all, pve_tiss, pve_bg, pve_1, pve_2, vars_cor,
	...
	#stopImplicitCluster()
	rownames(dat) <- NULL
	colnames(dat) <- c('Tissue','%VE', '%VE_tissue', '%VE_bg', '%VE_tissue_1', '%VE_tissue_2', 'Cor', 'BTeAo', 'mLog10PvalTeAo', 'BToAe', 'mLog10PvalToAe')
	print( dat )
