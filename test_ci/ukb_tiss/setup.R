...
get_pheno_covar_tiss <- function(phen, tissue, pvalue,remove_outliers=TRUE,qnorm=TRUE,eo_pc=FALSE,internal_prs_40,seed=0){
	...
	#### avg ts-prs
	load( paste0('Rdata/bg_TS_PRS/', pvalue, '_', phen, '_', type,  '.Rdata') )
	if( !phen %in% binphens ){
		prs$Pheno	<- NULL
	} else {
	...
	TS_All	<- rowSums( prs[,paste0(tissue, '_'	,c(chrsE,chrsO)	)] )
	TS_Even	<- rowSums( prs[,paste0(tissue, '_'	,chrsE					)] )
	TS_Odd	<- rowSums( prs[,paste0(tissue, '_'	,chrsO					)] )

	prs						<- cbind( prs[,c('IID','FID')], TS_All, TS_Even, TS_Odd, BG_All, BG_Even, BG_Odd )
	colnames(prs)	<- c( 'IID', 'FID', paste0( tissue, '_', c( 'All', 'Even', 'Odd' ) ), paste0( 'BG_', c( 'All', 'Even', 'Odd' ) ) )
