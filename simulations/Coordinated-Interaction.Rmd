...
```
We draw allele frequencies from a uniform distirbution, and draw SNPs from corresponding binomial distributions:
```{r}
set.seed(123)
...
..
```{r}
q1 = as.numeric(phenix:::quantnorm(as.matrix(y1)))
EOtest( betahatfxn(genos[trn,],q1[trn]), genos[tst,], q1[tst] )
```

### The Even-Odd test for different chromsome splits

The EO test power increases when the (arbitary) chromosome partition used with the causal pathway partition:
```{r}
